...
describe('global theme mode', () => {
  it('applies dark tokens to the document root', () => { expect(applyGlobalTheme('dark')).toBe('dark'); expect(document.documentElement.dataset.theme).toBe('dark'); expect(document.documentElement.style.getPropertyValue('--bg-primary')).toBe('#000000'); expect(document.documentElement.style.getPropertyValue('--border-custom')).toBe('#334155'); });

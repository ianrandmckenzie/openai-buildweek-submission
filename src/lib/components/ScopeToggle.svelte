<script lang="ts">
  ...
  export let scope: 'project' | 'all' | undefined;
  export let onChange: ((value: 'project' | 'all') => void) | undefined;

  $: currentScope = scope ?? ($activeView === 'quicknotes' ? $quicknotesScope : $projectViewMode);
  function setScope(value: 'project' | 'all'): void {
    if (onChange) onChange(value); else if ($activeView === 'quicknotes') quicknotesScope.set(value); else setProjectViewMode(value);
  }
</script>
<div class="scope-toggle" role="group" aria-label="Project scope"><button class:active={currentScope === 'project'} on:click={() => setScope('project')}>This project</button><button class:active={currentScope === 'all'} on:click={() => setScope('all')}>All projects</button></div>

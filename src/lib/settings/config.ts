...
export interface DashboardExport { format: 'kenzie-deskclerk'; version: 1; exported_at: number; stores: Partial<{ [K in StoreName]: RecordByStore[K][] }>; }

export function createDashboardExport(data: Partial<{ [K in StoreName]: RecordByStore[K][] }>, exportedAt = Date.now()): DashboardExport { return { format: 'kenzie-deskclerk', version: 1, exported_at: exportedAt, stores: data }; }
...
export function parseDashboardExport(serialized: string): DashboardExport {
  ...
  if (candidate.format !== 'kenzie-deskclerk' || candidate.version !== 1 || !candidate.stores || typeof candidate.stores !== 'object') throw new Error('Unsupported dashboard configuration format');
